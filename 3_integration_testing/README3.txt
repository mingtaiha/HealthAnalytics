All integration testing was done by hand.

The mobile application (card.io) tests integration and unit tests at the same time, 
refer to the 'services' test file to see how we run all requests to the processor.
`1_code/card.io/tests/Services/services.tests.js`. 

See the README file in `1_code/card.io` for details on how to run the test cases.

See the README file in `2_unit_testing/datastore` for information on how to run test cases for the processor/datastore. 